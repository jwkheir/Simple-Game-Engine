...
// GW WIP !!!!!!!!!!!!!!!!!!!!!!!!!!!!
SceneContext::SceneContext(FbxScene * scene, const char * pFileName, int pWindowWidth, int pWindowHeight, bool pSupportVBO)
: mFileName(pFileName), mStatus(UNLOADED),
mSdkManager(NULL), mScene(scene), mImporter(NULL), mCurrentAnimLayer(NULL), mSelectedNode(NULL),
...
void SceneContext::RenderNode(FbxNode * pRenderNode){
	...
	FbxNode * root = mScene->GetRootNode();
	 ...
	 FbxAMatrix lDummyGlobalPosition;
	 FbxPose * lPose = NULL;
//	 DrawNodeRecursive(pRenderNode, mCurrentTime, mCurrentAnimLayer, lDummyGlobalPosition, lPose, mShadingMode);
	 DrawNodeRecursive(root, mCurrentTime, mCurrentAnimLayer, lDummyGlobalPosition, lPose, mShadingMode);
